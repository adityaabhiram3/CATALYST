#!/bin/bash
# Sweep CATALYST tree workloads x pattern policies -- run on every node EXCEPT node 0.
# Start node 0 first; this follows the identical matrix so the two stay aligned.
#
#   cp ../script/run_catalyst*.sh ../script/sweep_catalyst*.sh \
#      ../script/catalyst_workloads.sh .
#   ./sweep_catalyst_other.sh
#
...
#
#   WORKLOADS="point_zipf scan_zipf" ./sweep_catalyst_other.sh   # subset
#   POLICIES="funnel model0" ./sweep_catalyst_other.sh           # subset
#   BULK=50 RUNNUM=50 ./sweep_catalyst_other.sh                  # larger runs
...
SUMMARY="${OUTDIR}/summary-other.tsv"
...
for w in $WORKLOADS; do
  ...
  for pol in $POLICIES; do
    ...
    env $(policy_env "$pol") \
        ...
        "${HERE}/run_catalyst_other.sh" > "$log" 2>&1
